import { UserManager, WebStorageStateStore, User, SignoutResponse } from 'oidc-client';
...
export class AuthorizeService {
    ...
    async completeSignOut(url: string) {
        ...
        try {
            assertIsDefined(this.userManager);
            const response = await this.userManager.signoutCallback(url) as SignoutResponse;
    ...
    unsubscribe(subscriptionId: number) {
        const subscriptionIndex = this._callbacks
            .map((element, index) => element.subscription === subscriptionId ? { found: true, index } : { found: false, index: 0 })
